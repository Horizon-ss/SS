...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;e.respondWith(caches.match(r).then(c=>c||fetch(r).then(res=>{const copy=res.clone();caches.open(STATIC).then(cache=>cache.put(r,copy));return res}).catch(()=>caches.match('./index.html'))))});